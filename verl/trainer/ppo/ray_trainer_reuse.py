...
class RayPPOTrainer:
    ...
    def _generate_multi_round_with_early_downsampling(
        ...
        # 片段缓存 & 结果累积
        first4_cache: dict[str, DataProto] = {}   # uid -> 第0轮的前4条片段
        pos_cache = defaultdict(list)             # uid -> [单行正样本片段, ...]
        neg_cache_by_round = defaultdict(lambda: defaultdict(list))  # uid -> round_idx -> [单行 DataProto,...]
        pos_cache_by_round = defaultdict(lambda: defaultdict(list))  # uid -> round_idx -> [单行 DataProto,...]
            ...
            for uid in list(active_uids):
                ...
                for j in locs:
                    ...
                    if seq_reward_np[j] > positive_threshold:
                        st["pos"] += 1
                        if r == 0 and j in st["first4_gidx"]:
                            if selected_count_by_uid.get(uid, 0) == 0 and not st["finished"]:
                                selected_pool_batches.append(first4_cache[uid])
                                selected_count_by_uid[uid] = final_keep_per_prompt
                                st["finished"] = True
                            made_positive_this_round += 1
                            break
                        else:
                            pos_cache[uid].append(mini_with_out[[j]])
                            pos_cache_by_round[uid][r].append(mini_with_out[[j]])  # 记录此轮的正样本
                            made_positive_this_round += 1
                    else:
                        # 记录负样本（单行片段）；注意保留所有轮次，兜底时用加权采样
                        neg_cache_by_round[uid][r].append(mini_with_out[[j]])

                # 达到比例后，用缓存片段收敛
                if not st["finished"]:
                    ratio = (st["pos"] / st["seen"]) if st["seen"] > 0 else 0.0
                    target_pos = 1 if ratio <= 0.375 else (2 if ratio <= 0.625 else 3)
                    target_pos = min(target_pos, final_keep_per_prompt - 1)
                    if len(pos_cache[uid]) >= target_pos and uid in first4_cache:
                        # 选正样本片段
                        ...
                        if neg_need > 0:
                            ...
                            idxs = list(range(min(neg_need, n_first4)))
                            if idxs:
                                frags_to_cat.append(first4_cache[uid][idxs])
        ...
        # === 新兜底：丢弃未出现 uid；在已出现 uid 中按“正样本越晚越大概率”加权随机抽 uid ===
        uids_missing = [u for u in uid_arr if selected_count_by_uid.get(u, 0) == 0]
        need_items = len(uids_missing)

        # 仅从“已出现过”的 uid 中抽
        uids_seen = [u for u in uid_arr if selected_count_by_uid.get(u, 0) > 0]

        if need_items > 0:
            if not uids_seen:
                print(f"[warn] 无已出现的 uid 可用于兜底重采样，放弃 {need_items} 个目标样本。")
            else:
                rng = np.random.default_rng()

                # 计算每个 donor uid 的“晚出现权重”：首个正样本的轮次越大，权重越高。
                donors = []
                weights = []
                for u in uids_seen:
                    pos_rounds = [rr for rr, lst in pos_cache_by_round[u].items() if len(lst) > 0]
                    if pos_rounds:
                        # 用“首个正样本”衡量出现时间（越晚越大）；如想用“最后一次正样本”，把 min 改为 max 即可。
                        first_pos_round = min(pos_rounds)
                        w = first_pos_round + 1  # 0..7 -> 权重 1..8
                    else:
                        # 若该 uid 没有记录到正样本，给极小权重（但仍允许被抽到）
                        w = 1e-6
                    donors.append(u)
                    weights.append(w)
                ...
                # 先一次性按权重“抽 need_items 个 uid”（可重复）
                chosen_uids = rng.choice(donors, size=need_items, replace=True, p=probs)

                def _make_pack_for_uid(u: str):
                    frags = []

                    # 1) 放入该 uid 的 1 个“正样本”
                    pos_rounds = [rr for rr, lst in pos_cache_by_round[u].items() if len(lst) > 0]
                    if pos_rounds:
                        # 偏好“更晚的正样本”——选择该 uid 的“最新一轮”的一个正样本
                        r_sel = max(pos_rounds)
                        pos_list = pos_cache_by_round[u][r_sel]
                        # 如果该轮已经被拿空，依次尝试更早轮
                        while r_sel is not None and (r_sel not in pos_cache_by_round[u] or len(pos_cache_by_round[u][r_sel]) == 0):
                            pos_rounds = [rr for rr in pos_rounds if rr < r_sel]
                            r_sel = max(pos_rounds) if pos_rounds else None
                        if r_sel is not None:
                            idx = rng.integers(0, len(pos_cache_by_round[u][r_sel]))
                            frags.append(pos_cache_by_round[u][r_sel].pop(idx))
                        else:
                            # 没有可用正样本了，跳过（也可以 fallback 用 first4 里的“看起来像正”的，但这里保持简单）
                            pass

                    # 2) 随机负样本（完全等概率、不看轮次），补到 final_keep_per_prompt
                    need_neg = final_keep_per_prompt - len(frags)
                    if need_neg > 0:
                        # 统计总负样本量
                        total_neg = sum(len(lst) for lst in neg_cache_by_round[u].values())
                        allow_reuse = True  # 如需严格补齐数量，建议允许复用

                        while need_neg > 0 and (total_neg > 0 or allow_reuse):
                            if total_neg == 0 and allow_reuse:
                                # 允许复用：从所有有负样本的轮里随机挑一轮，并随机取其中一条（不 pop）
                                rounds_with_neg = [rr for rr, lst in neg_cache_by_round[u].items() if len(lst) > 0]
                                if not rounds_with_neg:
                                    break
                                rr = rng.choice(rounds_with_neg)
                                lst = neg_cache_by_round[u][rr]
                                frags.append(lst[rng.integers(0, len(lst))])
                                need_neg -= 1
                                continue

                            # 等概率地在“总负样本池”里无放回选择一条（通过 pop 实现）
                            pick = rng.integers(0, total_neg)
                            acc = 0
                            chosen_rr = None
                            chosen_idx = None
                            for rr, lst in neg_cache_by_round[u].items():
                                if not lst:
                                    continue
                                if acc + len(lst) > pick:
                                    chosen_rr = rr
                            ...
                            if chosen_rr is None:
                                break
                            lst = neg_cache_by_round[u][chosen_rr]
                            frags.append(lst.pop(chosen_idx))
                            need_neg -= 1
                            total_neg -= 1

                    # 3) 若仍未满，用 first4_cache[u] 补位
                    remaining = final_keep_per_prompt - len(frags)
                    if remaining > 0 and u in first4_cache and first4_cache[u] is not None:
                        n_rows = _dp_rows(first4_cache[u])
                        take = min(remaining, n_rows)
                        if take > 0:
                            frags.append(first4_cache[u][:take])

                    if not frags:
                        return None
                    return _dp_cat(frags) if len(frags) > 1 else frags[0]

                produced = 0
                for u in chosen_uids:
                    pack = _make_pack_for_uid(u)
                    if pack is not None:
                        selected_pool_batches.append(pack)
                        produced += 1
                    else:
                        # 如果这个 uid 产包失败，尝试随机换一个 donor 再补一次
                        alt = rng.choice(donors, size=1, replace=True, p=probs)[0]
                        pack = _make_pack_for_uid(alt)
                        if pack is not None:
                            selected_pool_batches.append(pack)
                            produced += 1

                if produced < need_items:
                    print(f"[warn] 目标兜底数={need_items}，实际仅生成={produced}。如需严格一致，可放宽正样本/负样本复用。")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    actual_repeat = 32
                    round_repeat = 4           # 每轮为活跃 prompt 生成4条
