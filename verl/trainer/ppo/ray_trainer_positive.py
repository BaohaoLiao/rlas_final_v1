...
ray_trainer_gen8_balance_pos_only
"""
    ...
    def _generate_multi_round_with_early_downsampling(
            ...
            for uid in list(active_uids):
                ...
                if not st["finished"]:
                    ...
                    if len(pos_cache[uid]) >= target_pos:
                        ...
                        needed_neg = final_keep_per_prompt - target_pos
                        neg_frags = neg_cache[uid][:min(needed_neg, available_neg)]
                        
                        # 如果负样本不足，用额外的正样本补足到total=4
        ...
        for uid in uids_that_need_fallback:
            if uid in pos_cache or uid in neg_cache:
                ...
                if frags_to_merge:
                    merged = _dp_cat(frags_to_merge)
                    selected_pool_batches.append(merged)
                    selected_count_by_uid[uid] = _dp_rows(merged)
                else:
                    print(f"[WARN] uid={uid} frags_to_merge为空，无法兜底")
