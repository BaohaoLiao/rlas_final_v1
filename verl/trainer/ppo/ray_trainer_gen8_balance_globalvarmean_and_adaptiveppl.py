...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # 检查是否有全量统计信息（从多轮采样中获得）
        # 字典信息存储在meta_info中以避免reorder问题
...
class RayPPOTrainer:
    ...
    def _generate_multi_round_with_early_downsampling(
        ...
        assert actual_repeat % round_repeat == 0, "actual_repeat 必须能被 round_repeat 整除"
        max_rounds = actual_repeat // round_repeat
        target_pos = 4  # 至少需要4个正样本，给data selection更多空间
        target_neg = 2  # 至少需要2个负样本
        ...
        selected_pool_batches: list[DataProto] = []
        selected_count_by_uid = defaultdict(int)
        # fff新增：记录每个uid在所有轮次中的正负样本统计（用于GRPO计算真实mean/std）
        ...
        for r in range(max_rounds):
            ...
            for uid in list(active_uids):
                ...
                if not st["finished"]:
                    # 检查是否收集到足够样本（不进行选择，只是判断是否停止收集）
                    if len(pos_cache[uid]) >= target_pos and len(neg_cache[uid]) >= target_neg:
                        # 将所有收集到的样本加入候选池，不进行预选择
                        all_frags = pos_cache[uid] + neg_cache[uid]
                        merged = _dp_cat(all_frags)
                        selected_pool_batches.append(merged)
                        selected_count_by_uid[uid] = _dp_rows(merged)
                        st["finished"] = True
        ...
        # 兜底处理：将未完成uid的所有缓存样本加入候选池，让后续PPL自适应选择来决定
        for uid in uids_that_need_fallback:
            if uid in pos_cache or uid in neg_cache:
                # 简单地将所有缓存的正负样本都加入候选池
                all_cached_frags = pos_cache[uid] + neg_cache[uid]
                if all_cached_frags:
                    # 合并该uid的所有缓存样本，不进行任何选择
                    merged = _dp_cat(all_cached_frags)
                    selected_pool_batches.append(merged)
                    selected_count_by_uid[uid] = _dp_rows(merged)
                    print(f"[Fallback] uid={uid} 添加{len(all_cached_frags)}个缓存样本到候选池，等待PPL自适应选择")
                else:
                    print(f"[WARN] uid={uid} 没有缓存样本")
            else:
                print(f"[WARN] uid={uid} 不在pos_cache或neg_cache中，无法兜底")

        if not selected_pool_batches:
            raise RuntimeError("早停后没有选中任何样本，请检查阈值/规则是否过严或数据是否异常")

        # 🚀 一次性计算所有候选样本的PPL并进行自适应选择
        all_candidates_cache = _dp_cat(selected_pool_batches)
        ...
        # 计算所有候选样本的old_log_prob和perplexity
        ...
        for uid in uid_arr:
            ...
            # 下采样目标：2正2负，总共4个
            downsample_target_pos = final_keep_per_prompt // 2  # 2个正样本
            downsample_target_neg = final_keep_per_prompt - downsample_target_pos  # 2个负样本
            
            actual_pos = min(pos_num, downsample_target_pos)
            actual_neg = min(neg_num, downsample_target_neg)
            # 如果一种样本不足，用另一种补齐
        ...
        # 添加全量统计信息到 final_batch，用于GRPO计算真实mean/std
        # 使用uid作为键的字典形式，更安全可靠
        # 注意：字典不能被reorder，所以放到meta_info中
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if "token_level_rewards" in batch.batch:
                        final_pos_count = sum(1 for i in range(len(final_batch)) if final_batch.batch["token_level_rewards"][i].sum() > positive_threshold)
                        final_neg_count = len(final_batch) - final_pos_count
                        print(f"\n📈 [Step {self.global_steps}] 最终训练样本统计:")
                        print(f"   最终样本分布: {final_pos_count}正 + {final_neg_count}负 = {len(final_batch)}总")
                    else:
                        print(f"\n📈 [Step {self.global_steps}] 最终训练样本统计:")
                        print(f"   最终样本数量: {len(final_batch)}总 (无奖励信息)")
                    ...
                    # old_log_probs already computed in PPL calculation phase, add entropy metrics
                    if "entropy" in batch.batch:
